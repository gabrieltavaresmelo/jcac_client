...
import br.com.gtmf.controller.ClientController;
...
//		System.out.println(randomImageView.getId());
	...
	public void setConnected(final boolean isConnected) {
		...
		Platform.runLater(new Runnable() {
	        ...
	        public void run() {						
	        	if(isConnected){
	    			String username = ClientController.getInstance().getUsername();
	    			String ipPort = ClientController.getInstance().getIpServer();
	    			...
	    			ClientController.getInstance().destroy();
	    		}
	        }
		});		
	}
...
//			LOG.debug(json);			
			ClientController.getInstance().send(json);
